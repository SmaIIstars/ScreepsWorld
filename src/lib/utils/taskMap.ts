...
export type GeneratingPayload = {
  ...
  number?: number;
  memoryRoleOpts?: BaseRoleType & { bornRoom?: string };
  role: CustomRoleType;
};
...
export class TaskMap {
  ...
  public saveToMemory = () => {
    ...
    Memory.rooms[this.roomName].taskMapObj = this.getTaskMapObj();
  ...
  taskPriorityQueue(
    ...
    const availableTaskList = this.getByType(targetTaskType).filter((task) => {
    ...
    if (!targetPriorityList) {
      ...
      return availableTaskList.sort((a, b) => (a.timestamp ?? 0) - (b.timestamp ?? 0));
    } else {
