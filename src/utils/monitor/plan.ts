...
import { queryAvailableGetSourcePositions } from "../query";
...
const miner = () => {
  ...
  // 查找资源周围可用空间
  const allAvailablePositions = sources.reduce<Array<{ x: number; y: number }>>(
    (acc, source) => {
      const positions = queryAvailableGetSourcePositions(
        source.pos.x,
        source.pos.y
      );
